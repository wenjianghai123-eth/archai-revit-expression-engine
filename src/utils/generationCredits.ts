...
export function getGenerationOutputCount(mode: GenerationMode, config: object = {}): number {
  ...
  if (mode === 'floorplan') {
    ...
    if (outputMode === 'multi') return isFloorplanMultiPlanBatchCount(batchCount) ? batchCount : 1;
    return 1;
  }
  if (mode === 'design-variants') {
    return isDesignVariantBatchCount(batchCount) ? batchCount : 1;
  }
...
function isFloorplanMultiPlanBatchCount(value: unknown): value is 1 | 2 | 4 | 6 {
