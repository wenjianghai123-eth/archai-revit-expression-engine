...
import { AssetModel, PromptTemplate } from '../types';
...
import { EnterpriseKnowledgeLibrary } from './knowledge/EnterpriseKnowledgeLibrary';

const ModelPreview = lazy(() => import('./AssetModelPreview').then(module => ({ default: module.AssetModelPreview })));
...
function AssetModelPreviewLoading() {
  return (
    <div className="flex h-full min-h-[320px] w-full flex-col items-center justify-center gap-4 bg-slate-50">
      <div className="h-11 w-11 animate-spin rounded-full border-4 border-blue-100 border-t-blue-500" />
      <p className="text-xs font-bold text-slate-500">正在按需加载三维预览…</p>
    </div>
  );
}
...
export function AssetBank({
  ...
  const [view, setView] = useState<'knowledge' | 'models'>('knowledge');
  if (view === 'knowledge') {
    return (
      <EnterpriseKnowledgeLibrary
        ...
        onOpenModelLibrary={() => setView('models')}
      />
    );
  }
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      <div className="shrink-0 border-b border-slate-200 bg-white px-3 py-2">
        <button type="button" onClick={() => setView('knowledge')} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-black text-slate-700 hover:border-blue-300 hover:text-blue-700">
...
function ModelAssetBank() {
  ...
  useEffect(() => {
    const hasProcessingModel = models.some(model => model.conversionStatus === 'converting'
      || model.optimizationStatus === 'pending'
      || model.optimizationStatus === 'processing');
    if (!hasProcessingModel) return;
    const interval = window.setInterval(() => {
      void listModelAssets().then(records => {
        const nextModels = records.map(mapModelAssetRecord);
        setModels(nextModels);
        setSelectedAsset(current => nextModels.find(model => model.id === current?.id) || current);
        setDetailAsset(current => nextModels.find(model => model.id === current?.id) || current);
      }).catch(error => console.error('[model-assets] processing status refresh failed', {
            ...
            <div className="fixed inset-y-0 right-0 z-40 flex w-full max-w-xl flex-col border-l border-slate-200 bg-white shadow-2xl">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto p-3 custom-scrollbar">
                <div className="h-[420px] min-h-[320px] w-full overflow-hidden rounded-xl border border-slate-100 bg-slate-100">
                  <Suspense fallback={<AssetModelPreviewLoading />}>
                    <ModelPreview asset={detailAsset} />
                  </Suspense>
                </div>
                <div className="mt-3">
                  <ModelProcessingStatus model={detailAsset} />
                </div>
